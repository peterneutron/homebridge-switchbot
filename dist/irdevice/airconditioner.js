...
export class AirConditioner extends irdeviceBase {
    ...
    constructor(platform, accessory, device) {
        ...
        // Set category
        accessory.category = 21 /* this.hap.Categories.AIR_CONDITIONER */;
        ...
        this.HeaterCooler = {
            ...
            Service: accessory.getService(this.hap.Service.HeaterCooler) ?? accessory.addService(this.hap.Service.HeaterCooler),
        ...
        accessory.context.HeaterCooler = this.HeaterCooler;
        ...
        if (this.device.meterType && this.device.meterId) {
            const meterUuid = this.platform.api.hap.uuid.generate(`${device.meterId}-${device.meterType}`);
            ...
            this.HumiditySensor = {
                ...
                Service: this.meter.getService(this.hap.Service.HumiditySensor) ?? this.meter.addService(this.hap.Service.HumiditySensor),
    ...
    async UpdateCurrentHeaterCoolerState() {
        if (this.HeaterCooler.Active === this.hap.Characteristic.Active.ACTIVE) {
            ...
            if (this.device.hide_automode) {
                this.hide_automode = this.device.hide_automode;
            ...
            if (this.device.set_max_heat) {
                this.set_max_heat = this.device.set_max_heat;
            ...
            if (this.device.set_min_heat) {
                this.set_min_heat = this.device.set_min_heat;
            ...
            if (this.device.set_max_cool) {
                this.set_max_cool = this.device.set_max_cool;
            ...
            if (this.device.set_min_cool) {
                this.set_min_cool = this.device.set_min_cool;
    ...
    async getAirConditionerConfigSettings(accessory, device) {
        accessory.context.hide_automode = this.hide_automode = device.hide_automode;
        accessory.context.set_max_heat = this.set_max_heat = device.set_max_heat ?? 35;
        accessory.context.set_min_heat = this.set_min_heat = device.set_min_heat ?? 0;
        accessory.context.set_max_cool = this.set_max_cool = device.set_max_cool ?? 35;
        accessory.context.set_min_cool = this.set_min_cool = device.set_min_cool ?? 0;
    }
}
