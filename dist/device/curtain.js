...
export class Curtain extends deviceBase {
    ...
    constructor(platform, accessory, device) {
        ...
        if (device.hide_lightsensor || !isCurtainDevice(device)) {
        ...
        if (!device.silentModeSwitch) {
            ...
            this.OpenModeSwitch = {
                Name: name,
                Service: accessory.getService(name) ?? accessory.addService(this.hap.Service.Switch, name, uuid),
        ...
        if (!device.silentModeSwitch) {
            ...
            this.CloseModeSwitch = {
                Name: name,
                Service: this.accessory.getService(name) ?? accessory.addService(this.hap.Service.Switch, name, uuid),
    ...
    async BLEparseStatus() {
        ...
        if (!this.device.hide_lightsensor && this.LightSensor?.Service && 'lightLevel' in this.serviceData) {
    ...
    async setPerformance() {
        ...
        if (Number(this.WindowCovering.TargetPosition) > 50) {
            ...
            else if (this.device.setCloseMode === '0' || !this.CloseModeSwitch?.On) {
                setPositionMode = 0;
                Mode = 'Performance Mode';
            }
            else {
