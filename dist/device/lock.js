...
export class Lock extends deviceBase {
    ...
    constructor(platform, accessory, device) {
        ...
        if (device.hide_contactsensor) {
            ...
            this.ContactSensor = {
                ...
                Service: accessory.getService(this.hap.Service.ContactSensor) ?? this.accessory.addService(this.hap.Service.ContactSensor),
        ...
        if (device.activate_latchbutton === false) {
            if (this.Switch) {
    ...
    async BLEparseStatus() {
        ...
        // Contact Sensor
        if (!this.device.hide_contactsensor && this.ContactSensor?.Service) {
                        ...
                        return await this.retryBLE({
                            ...
                            fn: async () => {
                                if (this.LockMechanism.LockTargetState === this.hap.Characteristic.LockTargetState.SECURED) {
                                    return await device_list[0].lock();
                                }
                                else {
                                    return await device_list[0].unlock();
                                }
                            },
                        });
                    })
    ...
    async OnSet(value) {
        ...
        if (value) {
            ...
            this.openAPIpushChanges(value).then(async () => {
